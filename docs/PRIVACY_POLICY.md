...
本扩展以**本地优先**方式运行。部分联网功能为**默认启用或默认可用**，包括搜索建议和今日诗词；其他联网功能则通常需要您自行启用或配置，例如在线壁纸、部分第三方内容源或浏览器账号同步。无论是否启用上述功能，扩展都不会主动向开发者服务器上传您的数据。当前版本代码中，开发者**不运营用于接收扩展用户个人数据的专用后端服务**。
