...
本扩展以**本地优先**方式运行。部分联网功能为**默认启用或默认可用**，包括搜索建议和今日诗词；其他联网功能则通常需要您自行启用或配置，例如在线壁纸或部分第三方内容源。无论是否启用上述功能，扩展都不会主动向开发者服务器上传您的数据。当前版本代码中，开发者**不运营用于接收扩展用户个人数据的专用后端服务**。
...
对于浏览器应用商店、浏览器厂商保存的历史同步数据、第三方 API 或您自行填写的在线资源地址，其各自运营方可能作为**独立的数据处理者/控制者**处理相关数据；该等处理不由本扩展开发者直接控制。
...
2. **历史同步数据退役**：当前版本可能读取、下载或按您的操作清除旧版本留在浏览器同步存储中的数据，但不会再上传或应用这些数据。
3. **用户自选 WebDAV 同步**：仅在您主动配置并开启实验性 WebDAV 同步后，扩展才会直接连接您指定的服务器。
4. **第三方服务请求**：您启用联网功能时，您的浏览器直接向第三方服务发起的请求。
5. **应用商店分发**：Chrome Web Store、Microsoft Edge Add-ons、Firefox Add-ons 等平台在分发、安装、更新或统计时可能自行处理的数据。
...
- 快速导航链接、书签展示配置、常用网站相关本地状态；
- 为展示快速导航链接、常用网站或相关卡片而在本地处理的标签页标题、页面 URL、域名及 favicon 相关缓存；
- 搜索历史（如果相应功能在本地启用）；
- 本地壁纸、视频壁纸、Bing 壁纸缓存及相关元数据；
- 旧版本留下的设备名称、设备标识、同步时间戳、同步版本号等历史同步元数据；
- 旧版本留下的自定义搜索引擎列表、设置快照和快速导航数据。
- 如您启用 WebDAV，同步连接地址、用户名、设备的随机标识与名称、同步范围、基线、任务状态和脱敏错误；WebDAV 密码默认仅保存于当前浏览器配置文件，也可选择仅在本次浏览器会话中保存；
- 如您启用客户端加密，用于后台同步的不可导出派生密钥。独立同步加密密码本身不会上传、导出或写入日志。
...
- **storage**：用于保存设置与缓存，以及临时检测、下载或清除旧版本留下的同步数据；
- **tabs / activeTab / scripting（不同浏览器下）**：用于与标签页、新标签页和相关页面交互；
- **可选主机与请求检测权限**：当您启用某些需要访问网络资源的功能时，浏览器可能要求您授权指定主机或任意网页资源，例如拉取 favicon 或在线图片。仅在您测试 WebDAV 连接时，扩展会申请该服务器主机及 `webRequest` 可选权限；后者只用于识别和校验 WebDAV 重定向，不读取响应正文。

这些权限主要用于本地功能实现。例如，扩展可能从您访问过或当前打开的标签页中读取**页面标题、页面 URL/域名和 favicon 信息**，以便展示快速导航链接、常用网站或相关站点卡片；相关信息原则上在您的浏览器本地处理，不代表开发者会接收相应内容。
...
- 扩展设置、快速导航链接或壁纸文件，**但您主动开启 WebDAV 同步并选择相应同步范围时，所选内容会直接发送到您指定的 WebDAV 服务商，而不是开发者服务器**、

如果浏览器厂商仍保存旧版本留下的同步数据，或您启用了第三方服务请求，相关数据可能由**浏览器厂商或第三方服务提供方**独立接收和处理。
...
### 6.6 用户自选 WebDAV 同步（实验性）

- 默认状态：**关闭**；只有您填写服务器地址、用户名和密码并确认后才会连接；
- 用途：在您的设备之间双向同步您选择的扩展设置、快速导航、自定义搜索引擎、界面偏好以及可选的隐藏站点、在线壁纸地址、用户选择的图标和当前静态图片壁纸；
- 永不上传：WebDAV 密码、独立同步加密密码、浏览器权限、浏览器书签、视频壁纸、Bing/在线壁纸缓存、favicon 缓存、对象 URL 和旧版浏览器同步残留；
- 服务器可见信息：即使开启客户端加密，WebDAV 服务商通常仍可看到您的账号、连接 IP、请求时间、目录存在、文件数量和传输大小；
- 加密边界：客户端加密默认关闭。创建同步库时开启后，选定同步内容在上传前使用您的独立密码加密；同步库建立后不能更改加密状态或加密密码，开发者也无法恢复遗忘的密码；
- HTTP 风险：默认要求 HTTPS，仅局域网地址可在明确确认后使用 HTTP；外部公网 HTTP 不受支持。局域网 HTTP 仍可能使登录凭据和未加密内容被同一网络中的其他人读取；客户端内容加密也不能保护 WebDAV 登录凭据和全部网络元数据；
- 第三方责任：服务器由您或您选择的服务商控制。开发者无法保证其安全性、可用性、数据地域、日志、备份、删除传播或法律合规性。请优先使用 HTTPS、应用专用密码和可信服务商。
...
- 提供新标签页核心功能；
- 保存您的本地个性化设置和缓存；
- 检测、下载或清除旧版本留下的浏览器同步数据；
- 按您的主动选择，将指定数据同步到您配置的 WebDAV 服务器，并进行冲突检测、版本保留、完整性校验和客户端加密；
...
对于会使您的浏览器向第三方发送请求的联网功能，尤其是**默认启用或默认可用**的搜索建议和今日诗词，本扩展将其视为基于您的**同意**运行。您可通过禁用相关功能、切换至不联网替代功能、阻止对应域名请求、清除相关缓存或卸载扩展来撤回该同意。

对于您主动填写的在线壁纸、媒体 URL、第三方 API 地址或其他自定义联网资源，请求行为同样基于您的同意。

对于实验性 WebDAV 同步，连接服务器、保存本机凭据以及上传所选内容同样基于您的主动选择和同意。您可通过关闭同步并清除本机连接配置来停止后续处理；如需删除服务商已保存的数据，应使用扩展的“删除云端数据并断开”操作或服务商提供的管理工具。
...
当前版本不再提供浏览器账号云同步。旧版本留下的数据仍可能由浏览器厂商按其同步策略保留；扩展会暂时提供检测、下载和清除入口，但无法保证浏览器厂商何时完成跨设备传播。您也可以通过浏览器账号或同步数据管理功能进行清理。

### 9.3 WebDAV 同步数据

WebDAV 普通历史最多保留最近 10 个版本，并删除服务器修改时间超过 180 天且不属于当前或上一版本的数据；删除墓碑最长保留 180 天；当前静态图片壁纸按内容哈希去重并只保留正在使用的资源。关闭同步默认不会删除远端数据。选择删除时，扩展只尝试删除带有正确所有权标记的扩展专属目录；删除失败时会保留本机连接配置供重试。服务商自己的备份、回收站、日志或副本可能继续按其政策保留，扩展无法保证即时或彻底清除。

### 9.4 第三方服务日志和缓存

对于今日诗词、一言、Bing、Google、百度以及您自定义的第三方资源，其日志、缓存、访问记录和保留期限由相应服务提供方决定。本扩展开发者通常无法获知其确切保留时间，也无法代表其执行删除。
...
- 您可通过卸载扩展、清除浏览器扩展存储、关闭联网功能、删除本地壁纸与搜索历史，以及使用扩展提供的历史云数据清理入口等方式行使大部分控制权；
- 对于 WebDAV 数据，您可调整同步范围、导出本地备份、保留云端并断开，或在二次确认后删除扩展专属远端目录；如服务商另有备份或日志，请直接向该服务商行使权利；
