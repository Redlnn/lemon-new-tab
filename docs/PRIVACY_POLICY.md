...
- 扩展设置、快速导航链接、壁纸文件或搜索历史，**但您主动开启 WebDAV 同步并选择相应同步范围时，所选内容会直接发送到您指定的 WebDAV 服务商，而不是开发者服务器**。
...
- 用途：在您的设备之间双向同步您选择的扩展设置、快速导航、自定义搜索引擎、界面偏好以及可选的搜索记录、隐藏站点和当前静态图片壁纸；
...
- 加密边界：客户端加密默认关闭。开启后，选定同步内容在上传前使用您的独立密码加密；开发者无法恢复遗忘的密码；
- HTTP 风险：默认要求 HTTPS。局域网 HTTP 必须明确确认；外部 HTTP 还要求输入指定确认文字。HTTP 可能使登录凭据和未加密内容被同一网络、网络运营方或中间人读取；客户端内容加密也不能保护 WebDAV 登录凭据和全部网络元数据；
...
WebDAV 普通历史默认保留最近 10 个版本，您可调整为 2 至 20 个；删除墓碑最长保留 180 天；当前静态图片壁纸按内容哈希去重并只保留正在使用的资源。关闭同步默认不会删除远端数据。选择删除时，扩展只尝试删除带有正确所有权标记的扩展专属目录；删除失败时会保留本机连接配置供重试。服务商自己的备份、回收站、日志或副本可能继续按其政策保留，扩展无法保证即时或彻底清除。
